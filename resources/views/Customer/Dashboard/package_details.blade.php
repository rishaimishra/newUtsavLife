    ...
    @endif
      <div class="row">
         <div class="col-md-6 col-12 my-3">
            ...
            {{--  <div class="show">
                <img src="{{url('/')}}/storage/app/public/package/{{@$service->image}}" id="show-img">
         ...
         <div class="col-md-6 col-12 my-3">
          <div class="products-details-content">
             <h1 class="new-heading mb-2">{{@$package->name}}</h1>
             <label for=""><b>Description:&nbsp;&nbsp;</b></label>
             @php
                  $countChar = strlen($package->description);
                  $first100Char = substr(@$package->description,0, 100);
                  $onward100Char = substr(@$package->description,100, $countChar);
               @endphp
             ...
             <div class="product-price">
                ...
                <span class="old-price"> ₹ {{@$package->price}}</span><br>
                <label for=""><b>Discount Price:&nbsp;&nbsp;</b></label>
                <span class="new-price">₹ <del>{{@$package->discount_price}}</del></span><br>
                <label for=""><b>Unit:&nbsp;&nbsp;</b></label>
                <span class="qty-price"> {{@$package->unit}}</span>
             </div><br>

             @if(!@Auth::user()->id)
             <div class="add-cart">
             <a class="add cartbutton" href="#" data-bs-toggle="modal" data-bs-target="#myModalviewNew2{{$package->id}}"><i class="fas fa-shopping-cart mr-5"></i>Add
             </a>
          </div>
...
{{-- rohit modal --}}
<div class="modal" id="myModalviewNew2{{$package->id}}">
       ...
       <div class="modal-content">
          ...
          <form action="{{ route('cust.add_to_cart.new') }}" method="POST" id="frm">
             ...
             <div class="modal-body">
                <h3 class="cart-heading">Package name: <b>{{$package->name}}</b></h3>
                <!-- <div class="col-md-6"> -->
                <input type="hidden" name="package_id" value="{{$package->id}}">
                @php
                $allCat=DB::table('category__cruds')->where('category_status','!=','D')->get();
                @endphp
                ...
                <div class="modal-design">
                   ...
                   <input type="number" class="quentity form-control" name="quantity" id="quantityModal{{$package->id}}" value="1" min="1" {{-- onchange="changeQuantity2({{$package->id}})" --}}>
                </div>
                <div class="modal-design">
