@extends('Customer.layouts.app')
@section('title')
    <title>Utsavlife | Customer | CART dsjkafsa</title>
@endsection
@include('Customer.includes.head-cart')
@include('Customer.includes.new_header')
@section('content')
    <div id="loading">
        <div id="loading-center">
            <div id="loading-center-absolute">
                <div class="object" id="object_one"></div>
                <div class="object" id="object_two"></div>
                <div class="object" id="object_three"></div>
                <div class="object" id="object_four"></div>
                <div class="object" id="object_five"></div>
            </div>
        </div>
    </div>
    <div class="common-overlay"></div>

    <div class="section_padding" style="background: #f6f6f6;">
        <div class="container">
          <div class="bred_cum">
            <ul>
              <li><a href="{{ url('/') }}">Home</a></li>
              <li><a href="">Cart</a></li>
            </ul>
          </div>
          @if(count(@$cart)>0)
          <form method="post" action="{{route('cust.buy')}}">
            @csrf
          <div class="row">
            <div class="col-md-7">
              <div class="cart_box">
                <div class="title">
                  <h4>My Cart ({{ count($cart) }})</h4>
                </div>
                <div class="cart_body">

                @foreach(@$cart as $row)
                <input type="hidden" id="days{{$row->id}}" name="days[]" min="1" value="{{$row->days}}" onchange="DayFun({{$row->id}})" style="max-width:80px">
                <input type="hidden" id="pd{{$row->id}}" name="pd" value="{{@$row->days}}" >
                <input type="hidden" name="base_price[]" id="base_price{{$row->id}}" value="{{$row->price}}">
                <input type="hidden" name="total_price_input[]" id="total_price_input{{$row->id}}" value="{{$row->total_price}}">
                <input type="hidden" name="cart_id[]" value="{{$row->id}}">

                  <div class="cart_flex mb-3">
                    <div class="cart_img">
                    @if(!empty($row->service_id))
                      <img src="{{url('/')}}/storage/app/public/service/{{@$row->serviceDetails->image}}">
                      @else
                      <img src="{{url('/')}}/storage/app/public/packages/{{@$row->packageDetails->image}}">
                      @endif
                    </div>
                    <div class="cart_qty_txt">
                        <h4>
                          Add Quantity
                          <a onclick="return confirm('Are you sure want to delete this item from cart ?')" href="{{route('cust.cart.delete',$row->id)}}" class="text-body " style="float:right;">
                            <i class="far fa-trash-alt"></i>
                          </a>
                        </h4>
                        <span class="mt-3">By Utsav</span>
                        <div class="qty">
                          <button type="button" id="prevbtn{{ $row->id }}" onclick="quantityFun({{$row->id}},'-')">
                            <i class="fa-solid fa-minus"></i>
                          </button>

                          <input type="text" id="quantity{{$row->id}}" name="quantity[]" readonly @if(!empty($row->quantity)) value="{{ $row->quantity }}" @else value="1" @endif>
                          <input type="hidden" id="pq{{$row->id}}" name="pq" value="{{@$row->quantity}}" >

                          <button type="button" onclick="quantityFun({{$row->id}},'+')">
                            <i class="fa-solid fa-plus"></i>
                          </button>
                        </div>
                        <small class="mb-2">
                            @if(!empty($row->service_id))
                          <a class="product-name mb-10 text-heading" href="{{route('cust.single.product',$row->serviceDetails->id)}}">
                            {{$row->serviceDetails->service}}
                          </a>
                          @else
                          <a class="product-name mb-10 text-heading" href="{{route('cust.single.product',$row->packageDetails->id)}}">
                            {{$row->packageDetails->name}}
                          </a>
                          @endif
                        </small>
                        {{--  <br><small>{{$row->price}}</small>
                        <br><small>{{$row->days}}</small>
                        <br><small>{{$row->quantity}}</small>  --}}
                      </div>

                  </div>
                  <div class="row">
                    <div class="col-md-6 mb-2">
                        <h6>Category</h6>
                        <p>{{$row->categoryDetails->category_name}}</p>
                    </div>
                    <div class="col-md-6 mb-2">
                        <h6>Shift</h6>
                        <p>@if($row->time=="M")Morning @elseif($row->time=="N") Night @else Full Day @endif</p>
                    </div>
                    <div class="col-md-6 mb-2">
                        <h6>Start Date</h6>
                        <p>{{$row->order_date}}</p>
                    </div>
                    <div class="col-md-6 mb-2">
                        <h6>End Date</h6>
                        <p>{{$row->order_end_date}}</p>
                    </div>

                    <div class="col-md-6 mb-2">
                        <h6>Days</h6>
                        <p>{{$row->days}}</p>
                    </div>
                    <div class="col-md-6 mb-2">
                        <h6>Price</h6>
                        <p>₹{{$row->price}}</p>
                    </div>
                </div>
                  <div class="cart_des mb-3">
                    @if(!empty($row->service_id))
                    <p>{!! $row->serviceDetails->description !!}</p>
                    @else
                    <p>{!!   $row->packageDetails->description !!}</p>
                    @endif
                  </div><hr>
                @endforeach
                  <div class="cart_btn">
                    <input type="hidden" name="subTotal" id="subTotal" value="{{@$total_price}}">
                    <input type="submit" value="Process to checkout" class="btn buy-product" name="">
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-5">
              <div class="cart_box">
                <div class="title">
                  <h4>Total Price <span>Distribution</span></h4>
                </div>
                <div class="cart_body">
                  <table class="table table-lg table-borderless">
                    <tbody>
                      <tr>
                        <td>Price ({{ count($cart) }} item)</td>
                        <th class="text-right ">₹ <span class="subTotalText">{{@$total_price}}</span></th>
                      </tr>
                    </tbody>
                  </table>
                  <div class="cart_footer_result">
                    <span>Total Amount</span>
                    <span class="">₹<span class="subTotalText">{{@$total_price}}</span></span>
                  </div>
                  <div class="cart_des mt-4">
                    <p>This is Photoshops version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem.</p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          </form>
          @else
          <div class="empty-state__icon">
            <img src="data:image/png;base64," alt="">
            <h4 class="empty-state__message">Your Utsavlife Cart Is Empty</h4>
            </div>
          @endif


        </div>
      </div>


@section('footer')
@include('Customer.includes.new_footer')
@endsection
@endsection
@section('script')
@include('Customer.includes.script')

<script>
    function quantityFun(id,operation) {

        var quantity = $("#quantity" + id).val();
        var basePrice = $("#base_price" + id).val();
        var days = $("#days" + id).val();

        //condition for quantity  for subtotal calculation
        var prevCountity = $("#pq" + id).val();

        var prevSubTotal = parseInt($("#subTotal").val());
        if (operation == "+") {
            var totalPrice = (parseInt(quantity) + 1) * basePrice * parseInt(days);
            $("#total_price" + id).html(totalPrice);
            $("#total_price_input" + id).val(totalPrice);
            var newSubtotal = prevSubTotal + ((quantity) - (quantity - 1)) * basePrice * parseInt(days);
        } else {
            var totalPrice = (parseInt(quantity) - 1) * basePrice * parseInt(days);
            $("#total_price" + id).html(totalPrice);
            $("#total_price_input" + id).val(totalPrice);
            if(quantity - 1 == 0)
            {
                var newSubtotal = prevSubTotal;
            }
            else
            {
                var newSubtotal = prevSubTotal - ((quantity) - (quantity - 1)) * basePrice * parseInt(days);
            }
        }

        $("#subTotal").val(newSubtotal);
        $(".subTotalText").html(newSubtotal);
        $("#pq" + id).val(quantity);
        if(operation == "+")
        {
            quantity = parseInt(quantity) + 1;
        }
        else
        {
            quantity = parseInt(quantity) - 1;
            if(quantity == 0)
            {
                quantity = 1;
            }
        }
        $("#quantity" + id).val(quantity);
    }
</script>



<script>
    function DayFun(id) {
        // alert(id);
        console.log(id);

        var quantity = $("#quantity" + id).val();
        var basePrice = $("#base_price" + id).val();
        var days = $("#days" + id).val();


        var totalPrice = parseInt(quantity) * basePrice * parseInt(days);
        $("#total_price" + id).html(totalPrice);
        $("#total_price_input" + id).val(totalPrice);


        // var prevCountity=$("#pq"+id).val();

        //condition for days  for subtotal calculation
        var prevDays = $("#pd" + id).val();
        var prevSubTotal = parseInt($("#subTotal").val());
        if (days > prevDays) {
            var newSubtotal = prevSubTotal + ((days) - (days - 1)) * basePrice * parseInt(quantity);
        } else {
            var newSubtotal = prevSubTotal - ((days) - (days - 1)) * basePrice * parseInt(quantity);
        }

        console.log(prevSubTotal, newSubtotal);
        $("#subTotal").val(newSubtotal);
        $(".subTotalText").html(newSubtotal);
        $("#pd" + id).val(days);
        // console.log(quantity,basePrice,totalPrice);
    }
</script>
@endsection
