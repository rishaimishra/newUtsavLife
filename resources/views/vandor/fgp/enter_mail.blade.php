...
@section('title')
<title>Utsavlife | Vendor | Forget password</title>
@endsection
{{-- @section('head') --}}
                    ...
                    <div class="loginwrap">
                       <br><br><br><br>
                        <div class="form_title">
                            <h4 class="mb-5">Vandor Forget password (Enter Email)</h4>
                            <p class="mb-30">Don't have an account? <a href="{{route('vandor.registration.view')}}">Create here</a></p>
                        </div>
                        @include('vandor.includes.message')
                        <form method="POST" action="{{ route('vandor.email.entered.code.generate') }}"  id="login_form">
                            @csrf
