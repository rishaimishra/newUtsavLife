...
@section('title')
<title>Go party | admin | service Edit</title>
@endsection
@section('left_part')
				...
				<div class="col-lg-12">
					<div>
						...
						<div class="panel panel-default panel-fill">
							...
							<div class="add-btn "><a href="{{route('admin.service.list')}}"><i class="icofont-minus-circle"></i> Back</a></div>
							 </div>
								<form role="form" action="{{route('admin.update.service')}}" id="frm" method="post" enctype="multipart/form-data">
									@csrf
									...
									<div class="form-group rm50">
										...
										<select class="form-group" name="category_id" id="category_id" >
											...
											@foreach($cat as $val)
											<option value="{{$val->id}}" @if($val->id==$service->serviceCategoryDetails->category_id) selected @endif> {{$val->category_name}}</option>
											@endforeach
										</select>
									</div>
									...
									<div class="form-group rm50">
										...
										<input type="text"  class="form-control"  placeholder="Enter service name"  name="service" value="{{$service->service}}" >
									</div>

									<div class="clearfix"></div>
									<div class="form-group rm50">
										<label for="title">service Description</label>
										<textarea  class="form-control"  placeholder="Enter service description"  name="description" >{{$service->description}}</textarea>
									</div>
									...
									<div class="form-group rm50">
										...
										<input type="number"  class="form-control"  placeholder="Enter Price"  name="price" value="{{$service->price}}" >
									</div>

										<div class="clearfix"></div>
									<div class="form-group rm50">
										<label for="title">Discount Price</label>
										<input type="number"  class="form-control"  placeholder="Enter Discount Price"  name="discount_price" value="{{$service->discount_price}}" >
									</div>
									...
									<div class="form-group rm50">
										...
										<input type="text"  class="form-control"  placeholder="Enter unity"  name="price_basis" value="{{$service->price_basis}}" >
									</div>
{{-- image --}}
									...
									<div class="form-group" style="margin-left: 30px;">
										...
										<div class="uplodimgfilimgs">
											<em><img src="{{url('/')}}/storage/app/public/service/{{@$service->image}}" alt="" style="width: 150px !important; height: 150px !important"></em>
										</div>
									</div>
