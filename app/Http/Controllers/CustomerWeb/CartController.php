...
**/
	...
	public function addToCartNew(Request $request){
        ...
        {
            $flag = Package::where('id',$request->package_id)->first();
            $service_id = null;
            $package_id = $flag->id;
        }
	...
	public function showCart(Request $request)
	{
		...
		$data['cart'] = Cart::with('serviceDetails','packageDetails','categoryDetails')->where('user_id',Auth::user()->id)->orderBy('id','desc')->get();
...
public function service_address(){
	...
	if(@Auth()->user()->id && @Auth::user()->role_id==2){
		...
		if($find){
            ...
            $countCartPrice = Cart::where('user_id',Auth::user()->id)->sum('total_price');
            if($countCartPrice > 0)
            {
                $total_price = $countCartPrice;
            }
            $data['total_price'] = $total_price;

            $previousUrl = URL::previous();
            $lastSegment = Str::afterLast($previousUrl, '/');
...
public function details_service($id){
    ...
    $all_images[] = [
        'type' => 'image',
        'url' => url('/').'/storage/app/public/service/'.$service->image
    ];
    ...
    if(!empty($service->additional_images))
    {
        ...
        foreach($explodeImages as $image)
        {
            $all_images[] = [
                'type' => 'image',
                'url' => url('/').'/storage/app/public/service/'.$image
            ];
        }
    }
    if(!empty($service->video_url))
    {
        $explodeVideos = explode(",",$service->video_url);
        foreach($explodeVideos as $video_url)
        {
            $urlExplode = explode("&",$video_url);
            $secondExplode = explode("/",$urlExplode[0]);
            $search = "?v=";
            if(strpos($secondExplode[3], $search))
            {
                $you_tube =  str_replace("watch?v=","",$secondExplode[3]);
            }else{
                $you_tube = str_replace("","",$secondExplode[3]);
            }
            $all_images[] = [
                'type' => 'youtube',
                'url' => $you_tube
...
{
	$srch=Package::where('id',$id)->first();
	if(!@$srch){
		return back()->with('error','Id not found.');
	}
    ...
    $all_images[] = [
        'type' => 'image',
        'url' => url('/').'/storage/app/public/packages/'.$package->image
    ];
    ...
    if(!empty($package->additional_images))
    {
        ...
        foreach($explodeImages as $image)
        {
            $all_images[] = [
                'type' => 'image',
                'url' => url('/').'/storage/app/public/packages/'.$image
            ];
        }
    }
    if(!empty($package->video_url))
    {
        $explodeVideos = explode(",",$package->video_url);
        foreach($explodeVideos as $video_url)
        {
            $urlExplode = explode("&",$video_url);
            $secondExplode = explode("/",$urlExplode[0]);
    ...
    $data['all_images'] = $all_images;
    // dd($data);
	return view('Customer.Dashboard.package_details')->with($data);
}
